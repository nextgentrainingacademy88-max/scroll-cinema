...
function mountScrollWorld(container, config) {
  ...
  // `config.copy === false` hands the text layer to a page-level enhancer (e.g. a GSAP
  // ScrollTrigger layer that owns word-by-word reveals). The engine then builds/drives
  ...
  function layout() {
    ...
    read();
    // Let page-level enhancers (e.g. a GSAP text layer) sync to the exact scroll
    // ranges the engine computed. Fired on mount and on every relayout/resize.
    container.dispatchEvent(new CustomEvent('sw:layout', {
      detail: { vh, sections: SECTIONS.map(s => ({ id: s.id, start: s._seg.start, end: s._seg.end })) }
    }));
  }
